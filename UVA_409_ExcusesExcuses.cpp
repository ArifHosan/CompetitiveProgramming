        ...
        FOR(i,0,E) {
            FOR(j,0,K) {
                ...
                while(Pos!=string::npos) {
                    char sp=L[i][Pos-1],ep=L[i][Pos+W[j].size()];
                    if(Pos==0) sp='1';
                    if(Pos+W[j].size()>L[i].size()) ep='2';
                    if(!isalpha(sp) && !isalpha(ep))
                        M[i]++;
                    Pos=L[i].find(W[j],Pos+1);
                }
            }
        }
        int Max=*max_element(M,M+E);
